...
class Database:
    ...
    def upsert_number_plate(self, number_plate, distance_from_launch):
        """Insert new number plate or update only timestamp if it already exists."""
        try:
            existing = self.collection.find_one({"number_plate": number_plate})
            if existing:
                # Update only timestamp if plate exists
                result = self.collection.update_one(
                    {"number_plate": number_plate},
                    {"$set": {"timestamp": datetime.utcnow()}}
                )
                print(f"Updated timestamp for existing plate: {number_plate}")
            else:
                # Insert new record with full details
                result = self.collection.insert_one({
                    "number_plate": number_plate,
                    "timestamp": datetime.utcnow(),
                    "distance_from_launch": float(distance_from_launch)
                })
                print(f"Inserted new number plate: {number_plate}")
            return True
        except Exception as e:
...
if __name__ == "__main__":
    try:
        print("Initializing MongoDB test...")
        ...
        # Test upserting a sample number plate with distance
        test_plate = "VIGGUMODEL"
        distance = 0.8
        print(f"\nTesting upsert for number plate: {test_plate} with distance: {distance}")
        upsert_result = db.upsert_number_plate(test_plate, distance)
        if upsert_result:
            print(f"✅ Upsert operation completed for number plate: {test_plate}")
        else:
            print(f"❌ Upsert operation failed for number plate: {test_plate}")
        ...
        if exists:
            print(f"⚠️ Number plate {test_plate_nonexistent} unexpectedly found in database.")
        else:
